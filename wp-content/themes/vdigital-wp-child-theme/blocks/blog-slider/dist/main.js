...
document.addEventListener("DOMContentLoaded", function () {
  var sliderContainers = document.querySelectorAll(".blog-slider");
  sliderContainers.forEach(function (container, index) {
    if (!container) return;
    var sliderContainer = container.querySelector(".blog-slider__items");
    if (!sliderContainer) return;
    var prevButton = container.querySelector(".button--navigate.prev");
    var nextButton = container.querySelector(".button--navigate.next");
    var navContainer = container.querySelector(".blog-slider__card__navigation .dots");
    if (sliderContainer.querySelectorAll(".blog-slider__items__item").length <= 1) {
      prevButton.style.display = "none";
      nextButton.style.display = "none";
      navContainer.style.display = "none";
      return;
    }
    blogSliders[index] = (0,_node_modules_tiny_slider_src_tiny_slider__WEBPACK_IMPORTED_MODULE_0__.tns)({
    ...
    blogSliders[index].events.on("indexChanged", function (info) {
      var slides_wrapper = document.getElementById(info.container.id);
      var slider_container = slides_wrapper;
      while (slider_container && !slider_container.classList.contains("blog-slider")) {
        slider_container = slider_container.parentNode;
      }
      var currentSlide = info.slideItems[info.index].querySelector(".blog-slider__items__item");
      var label = currentSlide.getAttribute("data-label") || "";
      var labelColor = currentSlide.getAttribute("data-label-color") || "";
      var labelElement = slider_container.querySelector(".blog-slider__label");
