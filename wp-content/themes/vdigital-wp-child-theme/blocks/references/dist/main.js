...
var referencesSliders = [];
function updateDots(slider_container, currentIndex, slider) {
  var dots = slider_container.querySelectorAll(".dots .dot");
...
function initializeSliders() {
  ...
  var sliderContainers = document.querySelectorAll(".references");
  sliderContainers.forEach(function (container, index) {
    if (!container) return;
    var sliderContainer = container.querySelector(".references__cards");
    if (!sliderContainer) return;
    var navContainer = container.querySelector(".references__cards__navigation .dots");
    var items = sliderContainer.querySelectorAll(".references__cards__card");
    if (sliderContainer.querySelectorAll(".references__cards__card").length <= 1) {
      navContainer.style.display = "none";
      return;
    }
    referencesSliders[index] = (0,_node_modules_tiny_slider_src_tiny_slider__WEBPACK_IMPORTED_MODULE_0__.tns)({
    ...
    referencesSliders[index].events.on("newBreakpointEnd", function (info) {
      var containerId = info.container.id;
      var slides_wrapper = document.getElementById(containerId);
      var slider_container = slides_wrapper;
      while (slider_container && !slider_container.classList.contains("references")) {
        slider_container = slider_container.parentNode;
      }
      updateDots(slider_container, info.index, referencesSliders[index]);
    });
    referencesSliders[index].events.on("indexChanged", function (info) {
      var containerId = info.container.id;
      var slides_wrapper = document.getElementById(containerId);
      var slider_container = slides_wrapper;
      while (slider_container && !slider_container.classList.contains("references")) {
  ...
  var interval = setInterval(function () {
    ...
    if (sliderContainers.length > 0) {
      sliderContainers.forEach(function (container) {
        var links = container.querySelectorAll(".references__cards__card");
